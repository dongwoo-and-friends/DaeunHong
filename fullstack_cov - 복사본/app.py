...
def show_movies():
    ...
    for movie in movies:
        movie['info_url'] = f"/info/{movie['info_url']}"
...
@app.route('/api/like/<string:movie_id>', methods=['POST'])
def like_movie(movie_id):
    ...
    if result.modified_count == 1:
        updated_movies = list(db.movies.find({'trashed': False}).sort([('likes', -1)]))
        return jsonify({'result': 'success', 'movies_list': updated_movies})
    else:
        return jsonify({'result': 'failure'})

@app.route('/info/<string:movie_id>', methods=['GET'])
def get_movie_info(movie_id):
    ...
    return jsonify({'result': 'success', 'movie_info': movie})
# 휴지통으로 영화 이동
@app.route('/api/trash/<movie_id>', methods=['POST'])
def trash_movie(movie_id):
     return jsonify({'result': 'success'})
#  휴지통에서 영화 목록 가져옴
# @app.route('/api/list/trash', methods=['GET'])
# def get_trash_list():
     #return jsonify({'result': 'success', 'movies_list': trash_movies})
